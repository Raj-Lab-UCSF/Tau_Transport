...
simstr = 'beta_gamma_curve_finerange'; % for saving the outputs
...
% 2e. Define ranges for frac and gamma, instantiate matrices for storing n
% and m
fraclist = 0.6;
gammalist = 2e-5 * 2.^linspace(-5,5,31);
if strcmp(resmesh,'coarse')
    n_mat = zeros(length(gammalist),length(fraclist),tsteps,250);
elseif strcmp(resmesh,'fine')
    n_mat = zeros(length(gammalist),length(fraclist),tsteps,1000);
end
m_mat = n_mat;
biasmat = zeros(length(gammalist),length(fraclist),tsteps);

% 3. Run TauTransportPDE
for i = 1:length(gammalist)
    for j = 1:length(fraclist)
        fprintf('Simulation %d/%d \n',j+(i-1)*length(fraclist),length(gammalist)*length(fraclist))
        gamma = gammalist(i);
        frac = fraclist(j);
