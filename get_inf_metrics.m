...
for i = 9:81
    for j = 1:5
        ...
        dir = '/Users/nbarron/Desktop/inference_results_2/output_data/output_';
        filename = [dir num2str(i) '_' num2str(j) '.mat'];

        sim_num_str = num2str(i);
        rep_num_str = num2str(j);

        sim_data_path = ['/Users/nbarron/Desktop/Tau_Transport/frac07_sims/output_data/sim_outputs_' sim_num_str '.mat'];
        
        if exist(filename, 'file') && exist(sim_data_path,'file')
            ...
            beta = 1e-6;
            gamma1 = output_struct.x0(1); %gamma_in;
            gamma2 = 0;
            %frac = params(4);
            frac = 0.7;
            lambda1 = output_struct.x0(2); %lambda_in;
            lambda2 = output_struct.x0(2); %lambda_in;
            delta = output_struct.x0(3); %delta_in;
            epsilon = output_struct.x0(4); %epsilon_in;
            ...
            N_seed = model_output.Predicted.N;
            FVal_seed = model_output.Predicted.F;
            W1_seed = model_output.Predicted.W1;

            output_struct_seed = struct;
            output_struct_seed.N_seed = N_seed;
            output_struct_seed.F_seed = FVal_seed;
            output_struct_seed.W_seed = W1_seed;

            dir_out = '/Users/nbarron/Desktop/inference_results_2/seed_preds/seed_pred_';
            savepath = [dir_out num2str(i) '_' num2str(j) '.mat'];

            save(savepath,'output_struct_seed');
