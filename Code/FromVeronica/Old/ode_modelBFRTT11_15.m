...
% C_adj_in=Conn(:,1)>0;
% CC_in=Conn(C_adj_in,1);
% init_tau=init_tau(init_tau~=0);
% CC=CC_in(init_tau~=0);
...
% %
  ...
  m=zeros(nroi,nt);
  n(:,1)=init_tau(:);
 A_in= zeros([nroi,size(n,2)]);
 A_out=zeros([nroi,size(n,2)]);
...
for h=1:1 %nt-1
 ...
 n_adj_out=n_adj_out.';
  ...
  A_in(:,h)=flux_calculator11_15(n_adj_in,n(:,h),1);

  A_out(:,h)=flux_calculator11_15(n(:,h),n_adj_out,0); 
 
 %n(:,h+1)=n(:,h)+(diag((A_in(:,:,h)*Conn_in.')) - diag(A_out(:,:,h)*Conn_out.'))*k+ beta*m(:,h)*k-gamma*(n(:,h).*n(:,h)+n(:,h).*m(:,h))*k;

 n(:,h+1)=n(:,h)+(A_in(:,h) - A_out(:,h))*k+ beta*m(:,h)*k-gamma*(n(:,h).*n(:,h)+n(:,h).*m(:,h))*k;
