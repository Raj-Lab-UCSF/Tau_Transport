function [W_1_flux,W_2_flux,R_ss,S_ss] = SingleEdgeWCalculator(matdir,network_flux,varargin)
% Updated 24/08/02
if nargin < 4
...
%     N1_mat = repmat(ip.Results.tau_xL,1,nroi);
%     N2_mat = repmat(ip.Results.tau_xL.',nroi,1);
...
% else
    ...
    for j = 1:nroi
        Ad_in = logical(Adj(:,j));
        C_1 = ip.Results.tau_xL(j);
        C_1 = repmat(C_1,1,length(Ad_in));
        C_1 = C_1.';
        i_app_0 = logical((ip.Results.tau_x0(:,j)+C_1==0).*(Ad_in));
        i_len = ones(nroi,1);
    ...
    %     W_2_flux(i_app_0,j) = W_1_0(2)*i_app_00;  % ones(length(i_app_00),1);
        i_app = logical(((ip.Results.tau_x0(:,j)>0)+(C_1>0)).*(Ad_in));
        B_1 = ip.Results.tau_x0(i_app,j);
