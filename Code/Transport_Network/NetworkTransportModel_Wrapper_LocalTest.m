...
%
% Should be used for local testing/debugging - long jobs need to be sent to
% a computing cluster

%% 1. Define directories for saving outputs
clear; clc;
curpath = '/Users/justintorok/Documents/MATLAB/Tau_Transport'; % Change to local Tau_Transport directory
p = genpath(curpath);
addpath(p);
...
simstr = 'hippocampus_testmat_for_single_edge_debug'; % for saving the outputs
use_sr = 0;
...
T = []; % default = 0.05
dt = []; % default = 0.005
% trange = [0:0.01:0.1]; % LH debug test
...
for i = 9
    ...
    paramnames_i = paramnamescell(i,:); % prevents broadcast warning 
