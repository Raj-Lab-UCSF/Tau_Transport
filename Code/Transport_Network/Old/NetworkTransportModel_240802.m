function model_outputs = NetworkTransportModel_240802(matdir,varargin)
...
%         diag((Conn*F_out)))*ip.Results.dt); 
    ...
    if any(N(:) < 0)
        % fprintf('Found negative N in Simulation %d!\n',ip.Results.sim_no)
        % fprintf('Min. negative val. = %.2d\n',min(N(:,h+1)))
        % minposN = min(nonzeros(N(:,h+1)));
