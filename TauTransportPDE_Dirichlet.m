function [n, m, xmesh, trange, jn, jm] = TauTransportPDE_Dirichlet(varargin)
% % % 0. Introduction
...
addParameter(ip, 'Bn_r', Bn_r_);
addParameter(ip, 'Bn_l', Bn_l_);
addParameter(ip, 'Bm_r', Bm_r_);
addParameter(ip, 'Bm_l', Bm_l_);
parse(ip, varargin{:});
...
% % --------------------------------------------------------------
    ...
    pl = [ul(1)-ip.Results.Bn_l; ul(2)-ip.Results.Bm_l];
    pr = [ur(1)-ip.Results.Bn_r; ur(2)-ip.Results.Bm_r];
    ql = [0; 0];
    qr = [0; 0];
    end
    % --------------------------------------------------------------
